...
def index():	
	...
	lista=["<tr><td>"+str(row.id)+"</td>"\
		   "<td>"+row.codigo+"</td>"\
		   "<td>"+row.nombre+"</td>"\
		   "<td>"+str(row.importe)+"</td>"\
		   "<td>"+str(row.comision)+"</td></tr>"\
			for row in row] 	
...
def handle():
	...
	if form.accepts(request.vars, session):			
			redirect(URL('producto','index',vars=dict(exito=1) ) ) 
	elif form.errors:			
			exito=0
			#redirect(URL('producto','index',vars=dict(exito=0) ) ) 
...
def eliminar():
	...
	if len(request.args) > 0:
		...
		if row:					
			...
			redirect(URL('producto','index',vars=dict(exito=2) ) ) 
	return
...
# Let's import the wrapper
	...
	diver_table = [['Id', 'Codigo', 'Nombre', 'Precio', 'Comision']] # this is the header row 

	for row in db(db.in_producto.id>0).select() :   
		diver_table.append([row.id, row.codigo, row.nombre, row.importe, row.comision]) # these are the other rows
