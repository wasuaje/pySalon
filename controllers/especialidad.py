...
def handle():
	...
	if len(request.args) > 0:
		...
		registro = 0
	form = SQLFORM(db[tabla], registro, deletable=True,  submit_button='Enviar' ,    delete_label='Click para borrar:',  next=URL(c='especialidad',  f='handle'), \
			message=T("Operacion Exitosa !"),  upload=URL('download'))	
	if form.accepts(request.vars, session):			
			redirect(URL('especialidad','index',vars=dict(exito=1) ) ) 
	elif form.errors:			
			exito=0
...
def eliminar():
	...
	if len(request.args) > 0:
		...
		if row:					
			...
			redirect(URL('especialidad','index',vars=dict(exito=2) ) ) 
	return
...
# Let's import the wrapper
	...
	doc = pdf.Pdf('Especialidades de Producto', 'wuelfhis asuaje')
	...
	doc.add_header('Especialidades de producto')
	...
	for row in db(db.cf_especialidad.id>0).select() :   
