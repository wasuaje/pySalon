...
def handle():
	...
	if len(request.args) > 0:
		...
		registro = 0
	form = SQLFORM(db[tabla], registro, deletable=True,  submit_button='Enviar' ,    delete_label='Click para borrar:',  next=URL(c='caja',  f='handle'), \
			message=T("Operacion Exitosa !"),  upload=URL('download'))	
	if form.accepts(request.vars, session):			
			redirect(URL('caja','index',vars=dict(exito=1) ) ) 
	elif form.errors:			
			exito=0
...
def eliminar():
	...
	if len(request.args) > 0:
		...
		if row:					
			...
			redirect(URL('caja','index',vars=dict(exito=2) ) ) 
	return
